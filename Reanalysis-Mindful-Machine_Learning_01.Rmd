...
Take the *whole* dataset and create folds.
...
```{r}
folds <- createMultiFolds(dat2$practice, k = k, times = t) ## t rep k-fold CV for performance evaluation
```
...
run_models_on_all_subsamples <- function(data, folds, 
                                         modellist, 
                                         traincontrol, 
                                         outcomevariable, ...)
  ...
  for (i in 1:length(folds)) {
    ...
    out[[i]] <- run_modellist(modellist = modellist,
                              trainsample = data[folds[[i]], ],
                              testsample = data[-folds[[i]], ],
                              traincontrol = traincontrol,
                              outcomevariable = outcomevariable)
  }
...
```{r}
write_rds(out, path = paste0(output_path, "model_performance_nested.rds"))
